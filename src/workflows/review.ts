...
//   4. Write the final result back to the agent's state
//
...
const SPECIALIST_PROMPTS: Record<
...
${JSON.stringify(allFindings, null, 2)}
      ...
      let parsed = CoordinatorOutputSchema.safeParse({
        ...out,
        verdict:
...
function fallbackVerdict(findings: Finding[]): Verdict {
  if (findings.some((f) => f.severity === "critical")) return "requested_changes";
